...
function renderDeviceIssues(devices){
  ...
  $("#deviceIssues").innerHTML=filtered.length?filtered.map((device,index)=>{const deviceLabel=device.parkingLotName||device.deviceId;return `<article class="device-issue"><span class="issue-rank">${String(index+1).padStart(2,"0")}</span><div class="issue-device" title="Device: ${esc(device.deviceId)}"><b>${esc(deviceLabel)}</b>${renderResultHistory(device.resultHistory,10,true)}</div><div class="issue-counts"><span class="success-count"><strong>${device.successfulCount}</strong> 成功</span>${device.nullVacancyCount?`<span><strong>${device.nullVacancyCount}</strong> 無法辨識</span>`:""}${device.failedCount?`<span><strong>${device.failedCount}</strong> 失敗</span>`:""}<time>${age(device.lastResultAt)}</time></div></article>`}).join(""):`<div class="empty">${searching?"找不到符合的裝置。":watchlistTab==="healthy"?"目前沒有成功結果較多的裝置。":"目前沒有需要注意的裝置。"}</div>`;
...
function render(s){ latest=s; const active=s.jobs.filter(j=>j.status==="running"); $("#activeCount").textContent=active.length; $("#completedCount").textContent=`${s.totals.completed}/${s.totals.discovered}`; $("#heroTitle").textContent=labels[s.status] || "Worker is online"; $("#heroCopy").textContent=active.length?`${active.length} job${active.length===1?" is":"s are"} moving through the processing floor.`:`Last poll ${age(s.lastPollAt)} · uptime ${age(s.startedAt).replace(" ago","")}`;
